...
class QuadraticBezier:
    def __init__(self, start, end, control, color):
        self.start = start #xy tuple
        self.end = end     #xy tuple
        self.control = control     #xy tuple
        self.color = color

    def strarray(self):
        return ['  <path stroke="%s" stroke-width="1" fill="none" d="M %s,%s Q %s,%s %s,%s" />' % 
            (colorstr(self.color), 
...
class Circle:
    def __init__(self,center,radius,color, title = None):
    ...
    def strarray(self):
        stra = ["  <circle cx=\"%d\" cy=\"%d\" r=\"%d\"\n" %\
                (self.center[0],self.center[1],self.radius),
                "    style=\"fill:%s;\"  />\n" % colorstr(self.color)]
        if not self.title == None:
            stra.append('<text fill="#1e1e1e" font-family="sans-serif" font-size="10" x="%s" y="%s" style="text-anchor: middle; stroke-width:0;">%s</text>' % (self.center[0],self.center[1]+3, self.title[:5]))
        return stra
...
def colorstr(rgb): return "#%x%x%x%x%x%x" % (rgb[0]/16, rgb[0]%16,rgb[1]/16, rgb[1]%16, rgb[2]/16, rgb[2]%16)
